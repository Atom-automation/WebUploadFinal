...
public class CommonKeywordLibrary {
	...
	public static void WAIT() {
		TestAttributes.TakeScreenShotFlag= false;
		try {
			String a[]=TestAttributes.Data.split("||");
			int WaitTimeInt = Integer.parseInt(a[8]) * 1000;
	...
	public static void CALLTEST() throws Throwable{
		...
		if (DataSplit.length == 4) {
			...
			FunctionLibrary.ExecuteSteps(DataSplit[0].trim(), DataSplit[1].trim(), DataSplit[2].trim(), DataSplit[3].trim(),0);
